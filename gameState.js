...
export class GameState {
  ...
  static generateDeck(isPlayerDeck, activeCode, nextGameGuaranteedCard, hadReflectionLastGame) {
    let deck = [];
    ...
    if (isPlayerDeck) { 
      ...
      if (activeCode === 'SAVIOR' && !deck.some(c => c.id === 'savior')) {
        ...
        for(let i = 0; i < specialCount && deck.length < 9; i++) {
          ...
          } while (deck.some(c => c.id === cardToAdd.id)); 
          deck.push(cardToAdd);
        }
        ...
        const normalDeck = GameState.generateNormalDeckContent(true); 
      ...
      deck = GameState.generateNormalDeckContent(true, deck); 
      ...
      const baseAIDeck = aiDecks[aiDecks.length -1];
      deck = baseAIDeck.map(card => {
        const isEnhanced = Math.random() < 0.05; 
        return {
          ...card,
  ...
  static generateNormalDeckContent(isPlayerDeck, currentDeck = []) {
    ...
    if (isPlayerDeck) { 
      ...
      if (!deck.some(c => c.id === 'reflection') && Math.random() < 0.01) {
    ...
    while (deck.length < 9) {
      ...
      } else if (rarity < 0.05 && (RARE_CARDS.length > 0)) { 
        cardPool = RARE_CARDS;
      } else if (rarity < 0.15 && (UNCOMMON_CARDS.length > 0)) { 
