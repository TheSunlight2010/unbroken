...
const BUSINESS_COMMENTS = {
  ...
  cardRelations: {
    ...
    moonlight: [
      ...
      "§̵̛̠͚̲̓̽̓͝ỳ̸̯̝͔͉͎̆̒͘͝s̷̩͕̎̈́̆͜t̷̨͍͎̹̏͒̎t̷̨͍͎̹̏͒̎t̵͈͍̩̿",
    ],
    awareness: [
...
export function showBusinessReview(playerScore, totalPlayers, allCards, nextGameCallback) {
  ...
  if (performanceCategory !== 'poor' && allCards && allCards.length > 0) { // Offer cards only for good/perfect performance
    cardOfferSection.style.display = 'block';
    const shuffledCards = [...allCards].sort(() => 0.5 - Math.random());
    const uniqueOfferedCards = [];
    for (const card of shuffledCards) {
      if (!uniqueOfferedCards.some(offered => offered.id === card.id) && card.id !== 'awareness' && card.id !== 'null') { // Avoid offering game-breaking cards or duplicates
        uniqueOfferedCards.push(card);
