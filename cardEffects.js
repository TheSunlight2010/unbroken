...
export class CardEffects {
  static applyAbility(card, game, playArea) {
    ...
    } else if (card.id === 'allie') {
      const worendyPlayed = playArea.some(p => p.card.id === 'worendy');
      if (worendyPlayed) {
        game.nextCardPenalty = 2;
        card.power += 2;
        
        const message = messages.special.allie[card.variant || 'normal'][
          Math.floor(Math.random() * messages.special.allie[card.variant || 'normal'].length)
        ];
        
        const effectMsg = document.createElement('div');
        effectMsg.className = 'win-message';
        effectMsg.innerHTML = `
          <div class="avatar" style="background-image: url(${card.image})"></div>
          <span>${message}</span>
        `;
        document.getElementById('play-area').appendChild(effectMsg);
        setTimeout(() => effectMsg.remove(), 2000);
