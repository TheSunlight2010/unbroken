...
export const RARE_CARDS = [
  ...
  },
  {
    ...
    ability: 'Power becomes random (4-14). Enhanced if power > 10. Might explode...',
    image: 'seven.png',
    rarity: 'rare',
    applyAbility: function(game, playArea) {
      // Calculate explosion chance based on remaining cards
      const remainingCards = game.players[0].deck.length;
      const explosionChance = 0.5 - (0.0555555556 * (9-remainingCards));
      ...
      if (Math.random() < explosionChance) {
        ...
        effectMsg.innerHTML = `
          ...
          <span>*BOOM* ...I may have miscalculated.</span>
        `;
        document.getElementById('play-area').appendChild(effectMsg);
        setTimeout(() => effectMsg.remove(), 2000);
        
        return; // Skip normal power calculation if exploded
      }

      // Original jackpot and power mechanics
...
export const SPECIAL_CASES = [
  ...
  },
  {
    ...
    name: 'Binx',
    power: 8,
    ability: 'Applies random effects to other cards with a 50% chance: either double their power or cancel their abilities.',
    image: 'binx.png',
    rarity: 'special',
    ...
    applyAbility: function(game, playArea) {
      playArea.forEach(played => {
        if (played.card.id !== 'binx' && !played.card.protected) {
          if (Math.random() < 0.5) {
            played.card.originalPower = played.card.power;
            played.card.applyAbility = function() {};
          } else {
            played.card.power *= 2;
          }
  ...
  },
  {
    ...
    name: 'The Sunlight',
    power: 99999999999,
    ability: 'Power level is "SUN". Cannot be copied.',
  ...
  },
  {
    ...
    name: 'Madness',
    power: 12,
    ability: 'CLOWN MULTIPLIES POWER FOR EACH MOUTH IN PLAY, HONK HONK.',
    image: 'madness.png',
    rarity: 'special',
    spawnChance: 0.04,
    applyAbility: function(game, playArea) {
      this.power *= playArea.length; // Each card in play represents another "mouth"
      
      const effectMsg = document.createElement('div');
      effectMsg.className = 'win-message';
      effectMsg.innerHTML = `
        <div class="avatar" style="background-image: url(madness.png)"></div>
        <span>YOU DO NOT KILL CLOWN. CLOWN KILLS YOU.</span>
      `;
      document.getElementById('play-area').appendChild(effectMsg);
      setTimeout(() => effectMsg.remove(), 2000);
    }
  },
  {
    ...
    name: 'Crystuf',
    power: 10,
    ability: 'Gains +2 power for each bone-related card in play',
    image: 'crystuf.png',
    rarity: 'special',
    spawnChance: 0.05,
    applyAbility: function(game, playArea) {
      const boneCards = playArea.filter(p => 
        ['mr_bones', 'fossil', 'herb'].includes(p.card.id)
      ).length;
      
      if (boneCards > 0) {
        this.power += (boneCards * 2);
        
        const effectMsg = document.createElement('div');
        effectMsg.className = 'win-message';
        effectMsg.innerHTML = `
          <div class="avatar" style="background-image: url(crystuf.png)"></div>
          <span>The bones remember their master...</span>
        `;
        document.getElementById('play-area').appendChild(effectMsg);
        setTimeout(() => effectMsg.remove(), 2000);
      }
  ...
  },
  {
    ...
    name: 'Thief',
    power: 9,
    ability: '*incoherent flesh noises*',
    image: 'thief.png',
    rarity: 'special',
    spawnChance: 0.03,
    applyAbility: function(game, playArea) {
      // Steals highest power from another card
      const highestCard = playArea.reduce((highest, current) => 
        current.card.power > highest.card.power && current.card.id !== 'thief' ? 
          current : highest
      , playArea[0]);
      
      if (highestCard.card.id !== 'thief') {
        const stolenPower = Math.floor(highestCard.card.power / 2);
        highestCard.card.power -= stolenPower;
        this.power += stolenPower;
        
        const effectMsg = document.createElement('div');
        effectMsg.className = 'win-message';
        effectMsg.innerHTML = `
          <div class="avatar" style="background-image: url(thief.png)"></div>
          <span>*flesh sounds intensify*</span>
        `;
        document.getElementById('play-area').appendChild(effectMsg);
        setTimeout(() => effectMsg.remove(), 2000);
      }
  ...
  },
  {
    ...
    name: 'Jaquavius',
    power: 11,
    ability: 'Nullifies all non-glitch cards\' abilities',
    image: 'jaquavius.png',
    rarity: 'special',
    spawnChance: 0.04,
    applyAbility: function(game, playArea) {
      playArea.forEach(played => {
        if (!['null', 'moonlight', 'awareness'].includes(played.card.id) && 
            !played.card.protected) {
          played.card.applyAbility = function() {};
  ...
  },
  {
    ...
    name: 'Swap!Serif',
    power: 8,
    ability: 'MWEH HEH HEH! Blue attacks do more damage!',
    image: 'swapserif.png',
    rarity: 'special',
    spawnChance: 0.05,
    applyAbility: function(game, playArea) {
      // "Blue" cards are any with blue in their art/theme
      const blueCards = playArea.filter(p => 
        ['aurora', 'aqua', 'frost', 'binx'].includes(p.card.id)
      );
      
      blueCards.forEach(played => {
        played.card.power += 3;
        
        const cardElement = document.querySelector(`[data-card-id="${played.card.id}"]`);
        if (cardElement) {
          cardElement.style.boxShadow = '0 0 10px #0000ff';
      ...
      if (blueCards.length > 0) {
        ...
        effectMsg.className = 'win-message';
        effectMsg.innerHTML = `
          <div class="avatar" style="background-image: url(swapserif.png)"></div>
          <span>MWEH HEH HEH! THE MAGNIFICENT SERIF STRIKES AGAIN!</span>
        `;
        document.getElementById('play-area').appendChild(effectMsg);
        setTimeout(() => effectMsg.remove(), 2000);
      }
  ...
  },
  {
    ...
    power: 16,
    powerDisplay: 'i',
    ability: 'Power is imaginary. Reality is optional.',
    image: 'imaginary.png',
    rarity: 'special',
    spawnChance: 0.03,
    applyAbility: function(game, playArea) {
      ...
      effectMsg.className = 'win-message';
      effectMsg.innerHTML = `
        <div class="avatar" style="background-image: url(imaginary.png)"></div>
        <span>√(-1) = i = win</span>
      `;
      document.getElementById('play-area').appendChild(effectMsg);
      setTimeout(() => effectMsg.remove(), 2000);
