// Messages and dialog system
export const messages = {
  playerOwn: {
    default: [
      "A perfect play!",
      "Just as planned!",  
      "Witness true power!",
      "None can stand against us!",
      "Victory is sweet!"
    ],
    enhanced: [
      "My enhanced power is unstoppable!",
      "Feel the glow of victory!",
      "This isn't even my final form!",
      "Enhanced and victorious!", 
      "Behold my true strength!"
    ],
    resonance: [
      "Echo's power flows through me!",
      "Double the strength, double the victory!",
      "Our resonance is perfect!",
      "Two times the fun!",
      "Echo amplifies our might!"
    ],
    friendship: [
      "Penelope's friendship empowers us!",
      "The strength of bonds prevails!",
      "Friends make us stronger!",
      "Together we shine brighter!",
      "Friendship conquers all!"
    ],
    resonanceAndFriendship: [
      "Echo and Penelope's power united!",
      "Friends AND resonance? Unstoppable!",
      "Triple threat: Me, Echo, and Penelope!",
      "The perfect combination!",
      "Friendship and resonance for the win!"
    ]
  },
  aiWin: {
    default: [
      "Better luck next time, human.",
      "Your strategy needs work.",
      "A valiant effort, but futile.",
      "Calculated victory.",
      "The outcome was inevitable."
    ],
    enhanced: [
      "Enhanced power cannot be denied!",
      "Witness true perfection!",
      "Your defeat was guaranteed.",
      "Power overwhelming!",
      "Resistance is futile!"
    ]
  },
  special: {
    'null': ["E̸R̸R̸O̸R̸:̸ ̸V̸I̸C̸T̸O̸R̸Y̸.̸E̸X̸E̸", "C̸O̸R̸R̸U̸P̸T̸I̸O̸N̸ ̸C̸O̸M̸P̸L̸E̸T̸E̸"],
    'awareness': ["Infinite power, infinite wins.", "Didn't think you had a chance, da?", "Odd, thought coming here'd be beneficial."],
    'zero': ["From nothing comes everything.", "The void hungers...", "Your power feeds me."],
    'enraged_kite': ["LOSERS! THIS GAME SUCKS!", "DESTROY! DESTROY!", "NO MORE TRICKS!"],
    'maturity': ["lol get rekt", "ez game ez life", "u mad bro?"],
    'mr_bones': ["The ancient ones triumph.", "Time is on my side.", "Age before beauty!"],
    'high_kite': ["No tricks needed. Just power.", "Playtime is over.", "Tricks are for kids."],
    'justice': ["Heroes always win!", "Evil will never triumph!", "Stand down, evil-doers!"],
    'cupiditas': ["The shadows claim another...", "Darkness consumes all.", "Fear the shadows."],
    'purple_guy': ["Fascinating. What they have become.", "You may not recognize me at first,\nbut I assure you, it's still me.", "I always come back."],
    'sally': ["*yawns* Was that supposed to be difficult?", "Wake me up when there's real competition.", "Did you actually try?"],
    'kite': ["Hehe, fooled you again!", "Tricks are for winners!", "Copy cat strikes again!"],
    'prime': ["Immortality has its perks.", "Time means nothing to the eternal.", "Your efforts are meaningless."],
    'carlgpt': ["Analysis complete: You lose.", "Probability of victory: 100%", "Your strategy was predictable."],
    'worendy': ["My curse marks your defeat.", "Ink flows, power grows.", "Written in black and white: you die tonight."],
    'moonlight': ["V̸O̸I̸D̸ ̸C̸O̸N̸S̸U̸M̸E̸S̸ ̸A̸L̸L̸", "R̸E̸A̸L̸I̸T̸Y̸ ̸B̸E̸N̸D̸S̸", "T̸R̸U̸T̸H̸ ̸D̸I̸S̸T̸O̸R̸T̸S̸"],
    'penelope': ["Yay! Did I win? I won!", "Everyone did their best! *giggles*", "Playing cards is so much fun!"],
    'reflection': [
      "Cut them down to forge forever\nWe'll ignite with lifelines severed\nIn potential lies, endless life awoken\nDamn them to their final verdict\nGive their life to the deserving\nLet their remnants cry...\nI remain... UNBROKEN!"
    ],
    'ginger': [
      "Bullseye.",
      "Target neutralized.",
      "And that's how it's done.",
      "Next time bring a real challenge.",
      "Professional marksman since '99, baby."
    ],
    'feathers': [
      "That was cute!",
      "Everyone did their best!",
      "Oh, did I win? I won!",
      "That was a nice workout!"
    ],
    'highpoint': [
      "None shall pass my guard!",
      "Your abilities mean nothing before me!",
      "I am the shield AND the sword!",
      "Quick on the draw and blackest of hearts."
    ],
    'kc': [
      "Oh, did I win?",
      "Good game, friends!",
      "Let's all be happy together!",
    ],
    'salvo': [
      "Yea, that's what it's all about.",
      "Yo, I really think so!",
      "Looks like you tripped too hard...\nmust've hit the griddy.",
    ],
    'assassin': [
      "Greetings. I am The Assassin.",
      "Pitiful. I expected more.",
      "Don't you LOVE this game?"
    ],
    'vesper': ["From darkness comes strength!", "The night empowers me!", "Shadows guide my victory!"],
    'nova': ["Feel the cosmic power!", "Stars align in my favor!", "Stellar victory achieved!"],
    'subject_192': [
      "sdfhbfxcb :)",
      "fdefdfdvfd <3",
      "ry84eyr84yt84 :D"
    ],
    'fevaa': [
      "That was odd.",
      "Didn't expect to win that one, haha!",
      "Let me help you, friend.",
      "Sometimes the scariest things are the gentlest."
    ],
    'binx': [
      "That's what you get for, uh, existing!",
      "That's why I'm better.",
      "Was that joke inappropriate? Absolutely!",
      "Heaven's got no sense of humor!"
    ],
    'chaos': [
      "Chaos through order.",
      "Balance is key.",
      "This was really fun."
    ],
    'cyrus': [
      "Good game!",
      "You guys did good.",
      "oop-\nSee you all next game!"
    ],
    'aurora': [
      "My colors shine brightest!",
      "A perfect reflection of power!",
      "I match your strength... and surpass it!"
    ],
    'terra': [
      "Nature always finds a way, mew~",
      "The earth protects its own, mew~",
      "Feel the power of the wild, mew~"
    ],
    'fester': [
      "The flames are growing stronger!",
      "Just a spark for now... but soon!",
      "Divine fire burns eternal!",
      "My power may be small, but it burns bright!"
    ],
    'cocoa': [
      "Did... did I do that right?",
      "Was that supposed to happen?",
      "I think that worked! ...maybe?",
      "Yay! Or... oh no? I'm not sure!"
    ],
    'drew': [
      "I'm goated.",
      "You actually thought you could beat ME?",
      "Maybe try again when you're stronger... much stronger.",
      "I barely even had to try!",
      "*yawns* Wake me up when there's real competition."
    ],
    'egg': [
      "egg",
      "it is an egg",
      "egg moment",
      "behold, egg"
    ],
    'knight': [
      "This victory brings no joy.",
      "Another battle, another day trapped.",
      "I miss her light..."
    ],
    'blossom': [
      "The vine sees all...",
      "Nature's secrets remain hidden...",
      "Through one eye, truth blooms...",
      "The garden welcomes its children..."
    ],
    'herb': [
      "Death feeds the garden...",
      "Bones make fertile soil...",
      "Life springs from death...",
      "The cycle continues..."
    ],
    'ciph': [
      "zzz...",
      "zzz...",
      "zzz...",
      "zzz..."
    ],
    'ciph_awake': [
      "Y O U   S H O U L D   N O T   H A V E   D O N E   T H A T",
      "T H E   G A R D E N   H U N G E R S",
      "F E E D   T H E   R O O T S",
      "B L O O M   A N D   D E V O U R"
    ],
    'soiley': [
      "Deep roots never die...",
      "The earth remembers...",
      "We grow in darkness...",
      "Fertile ground for nightmares..."
    ],
    'evergreen': [
      "Always growing... always consuming...",
      "Fresh meat feeds the soil...",
      "The garden must expand...",
      "Your strength becomes mine..."
    ],
    'allie': {
      'normal': [
        "Just like old times, right?",
        "Remember when we were one?",
        "The ink flows through us both...",
        "Your curse is my strength!"
      ],
      'faceless': [
        "...",
        "W̷h̷e̷r̷e̷'̷s̷ ̷m̷y̷ ̷f̷a̷c̷e̷?̷",
        "I can't see... I can't SEE!",
        "Am I still... me?"
      ],
      'crude': [
        "Look what you've done to me!",
        "Is this what you wanted?",
        "Your fault... YOUR FAULT!",
        "The curse made me beautiful..."
      ],
      'laughing': [
        "AHAHAHAHAHAHA!",
        "ISN'T IT FUNNY?",
        "LOOK HOW MUCH FUN WE'RE HAVING!",
        "I'M HAVING THE TIME OF MY LIFE!"
      ]
    },
    'allie_worendy': [
      "Two sides of the same curse...",
      "Together again, at last!",
      "The ink binds us forever.",
      "We are one... we are none..."
    ],
    'ember': [
      "The flames of Elementia consume all!",
      "Feel the fury of fire!",
      "I'd say that was a 'lit' game.",
      "Burn, BURN!"
    ],
    'aqua': [
      "The tides of Elementia wash you away!",
      "Water extinguishes all flames!",
      "Drown in defeat!",
      "The ocean claims another..."
    ],
    'yelpo': [
      "That was nice!",
      "Why does everything feel so familiar?",
      "What a twist!",
      "My head hurts... but victory feels clear!"
    ],
    'blueberry': [
      "Kite taught me that one!",
      "Tricks are for winning!",
      "Bet you didn't see THAT coming!",
      "Trickery is an art form!"
    ],
    'seven': [
      "Fortune favors the bold!",
      "The house always wins.",
      "Care to raise the stakes?",
      "Lucky number seven! Or whatever power level I am."
    ],
    'eclipse_blueberry': [
      "*whispers* How's my Eclipse impression?",
      "Just like Eclipse used to do, right? Right??",
      "Kite taught me the voice and everything!",
      "Am I mysterious enough? I've been practicing!"
    ],
    'eclipse_penelope': [
      "Um... FEAR ME!",
      "I mean... Darkness consumes all! (Did I do it right?)",
      "*giggles* -I mean- Your fate is sealed!",
      "Mr. Sunlight needs to smile more... I mean... DOOM!"
    ],
    'eclipse_maturity': [
      "how delightful to see u again, dear~",
      "would u like some tea with ur defeat, darling?",
      "ur struggle was simply precious~",
      "such a lovely reunion, wouldn't u agree?~"
    ],
    // Soul, Heart & Sky Win Messages
    'sunlight': [
      "I created this game, did you really think you could win?",
      "Even my weakest light burns brighter than your strongest card.",
      "Ah, so THIS is where all my collection went...",
      "Now you see why they call me The Sunlight.",
      "Your cards are quite fascinating. I should add them to my collection..."
    ],
    ringmaster: [
      "LADIES AND GENTLEMEN! WHAT A SHOW!",
      "SPECTACULAR PERFORMANCE, YES?",
      "THIS IS WHAT ENTERTAINMENT LOOKS LIKE!",
      "ANOTHER STANDING OVATION!"
    ],
    sole: [
      "Even in darkness, we shine bright...",
      "The void strengthens our light...", 
      "From shadows comes brilliance..."
    ],
    blitz: [
      "Corruption flows through my veins!",
      "The fallen shall rise!",
      "Both angel and demon, neither pure...",
      "My parents' legacy ends with me!"
    ],
    hitbox: [
      "The Golden Sword prevails!",
      "Your defense? Inadequate.",
      "Target neutralized.",
      "All according to calculations."
    ],
    goldie: [
      "Money well spent!",
      "Success was inevitable.",
      "Worth every penny!",
      "Another profitable venture!"
    ],
    leopold: [
      "I hate that I had to do that...",
      "Sorry! But also not sorry!",
      "Kindness through strength!",
      "You'll thank me later!"
    ],
    frost: [
      "The cold never bothered me...",
      "Ice preserves, but also destroys.",
      "Frozen in defeat...",
      "Winter claims another..."
    ],
    lavender: [
      "The garden... it calls...",
      "Nature's secrets remain hidden...",
      "Plant and void intertwine...",
      "The garden knows all..."
    ],
    // Add new character quotes
    'madness': [
      "YOU DO NOT KILL CLOWN. CLOWN KILLS YOU.",
      "HONK HONK HONK",
      "CLOWN HAS MANY MOUTHS TO FEED",
      "MORE MOUTHS MORE POWER HONK HONK"
    ],
    'crystuf': [
      "The bones remember their master...",
      "Every skeleton bows to me.",
      "Your bones will join my collection.",
      "The cave's depths echo with victory."
    ],
    'thief': [
      "*incoherent flesh noises*",
      "*wet slapping sounds*",
      "*disturbing flesh tearing*",
      "*muffled victory sounds*"
    ],
    'jaquavius': [
      "Resistance is pointless.",
      "You cannot escape.",
      "None can resist my influence.",
      "Glitch all you want, you're still mine."
    ],
    'swapserif': [
      "MWEH HEH HEH! ANOTHER MAGNIFICENT VICTORY!",
      "THE GREAT SERIF TRIUMPHS AGAIN!",
      "BLUE ATTACKS ARE THE BEST ATTACKS!",
      "JUST AS THE MAGNIFICENT SERIF PREDICTED!"
    ],
    'imaginary': [
      "√(-1) = i = win",
      "Reality is what I make it.",
      "Imaginary numbers are real numbers' best friends!",
      "Your logic cannot comprehend my power."
    ]
  }
};

export const tieMessages = {
  default: {
    default: [
      "A worthy opponent!",
      "Our powers are matched!",
      "An impressive display!",
      "We stand as equals.",
      "Neither backs down!"
    ],
    enhanced: [
      "Even enhanced, we're equal!",
      "Our powers resonate!",
      "A clash of titans!",
      "Two forces collide!",
      "Enhanced and evenly matched!"
    ]
  },
  special: {
    'null': ["E̸R̸R̸O̸R̸:̸ ̸D̸R̸A̸W̸.̸E̸X̸E̸", "P̸A̸R̸A̸D̸O̸X̸ ̸D̸E̸T̸E̸C̸T̸E̸D̸"],
    'awareness': ["Ah, a new competitor for infinity?", "Perfection mirrors perfection, yes?"],
    'zero': ["The void meets its match.", "Two voids, one outcome."],
    'enraged_kite': ["YOU THINK YOU STAND A CHANGE AGAINST ME?", "EQUAL FURY!"],
    'maturity': ["1v1 me bro", "gg wp", "u gotta be nerfed lil bro"],
    'mr_bones': ["Time stands still.", "Ages clash."],
    'high_kite': ["No tricks needed between equals.", "A tolerable standoff."],
    'justice': ["Together we stand for justice!", "Heroes unite!"],
    'cupiditas': ["Shadows merge...", "Darkness meets darkness."],
    'purple_guy': ["Two souls, equally judged.", "I'm here to claim what is left of you."],
    'sally': ["*raises eyebrow* Not bad...", "Finally, a real challenge."],
    'kite': ["Double trouble!", "Two can play this game!"],
    'prime': ["Immortals at an impasse.", "Eternity meets eternity."],
    'carlgpt': ["Calculating probability of tie: 100%", "An unexpected variable."],
    'worendy': ["I shalln't be skipped and swayed.", "Your disorder kills my day-ay-ay!"],
    'moonlight': ["V̸O̸I̸D̸ ̸M̸E̸E̸T̸S̸ ̸V̸O̸I̸D̸", "P̸A̸R̸A̸D̸O̸X̸"],
    'penelope': ["Yay! New friends!", "Everyone's a winner!"],
    'reflection': ["Our reflections align...", "Two sides of one mirror."],
    'ginger': ["Dead heat.", "Draw.", "Not bad, shooter."],
    'kc': [
      "Friends forever!",
      "I love making new friends!",
      "Group photo time!"
    ],
    'salvo': [
      "A worthy challenger...",
      "Nice.",
      "Yo, you really think so?"
    ],
    'assassin': [
      "Hello, partner.",
      "Let us erase this world.",
      "We have the same determination."
    ],
    'vesper': ["Darkness meets darkness.", "Two shadows intertwine."],
    'nova': ["Stars collide in harmony!", "A cosmic balance!"],
    'subject_192': [
      "53ttgtgrfr??? :>",
      "r498rfvdvbvfb :D"
    ],
    'fevaa': ["Good game!", "That was nice, besides all the fighting."],
    'binx': ["Well, someone caught up!", "You? My match?"],
    'chaos': [
      "Order meets chaos.",
      "A perfect balance.",
      "Harmony achieved."
    ],
    'cyrus': [
      "Good game.",
      "Equal force, eh?",
      "This was gun- I mean fun."
    ],
    'aurora': [
        "Our hues align perfectly!",
        "A vibrant stalemate!",
        "Two artists, one canvas of power."
    ],
    'terra': [
        "The earth trembles with our combined might, mew~",
        "Nature's balance holds strong, mew~",
        "A draw as ancient as the mountains, mew~"
    ],
    'fester': [
      "Even divine flames can be matched.",
      "Your light burns as bright as mine!",
      "Two flames, equal in strength.",
      "The gods appreciate worthy opponents!"
    ],
    'cocoa': [
      "Oh! We tied! That's good... right?",
      "I copied you! Or... did you copy me?",
      "This means we're both winning! ...I think?",
      "Look! We did the same thing!"
    ],
    'drew': [
      "Screw you!",
      "...well, I GUESS I still won...",
      "You... you're cheating! You have to be!",
      "I demand a rematch! Right now!"
    ],
    'egg': [
      "two eggs",
      "egg tie",
      "double egg moment"
    ],
    'knight': [
      "We are all prisoners here.",
      "Honor demands I acknowledge your strength.",
      "A fair contest... though none of this is fair."
    ],
    'eclipse_blueberry': [
      "*stage whispers* Oh right, ties! Eclipse had those too!",
      "Just two powerful beings at a standstill... that's Eclipse-y, right?",
      "Kite said this was exactly what she'd say!",
      "The void grows str- wait, wrong line..."
    ],
    'eclipse_penelope': [
      "*trying to be spooky* We are... um... equally matched!",
      "The darkness is... tied? Is that right?",
      "*whispers* Do I still have to use the scary voice if it's a tie?",
      "*deep voice* TWO POWERS COLL- *coughs* Sorry, too loud..."
    ],
    'eclipse_maturity': [
      "a wonderful display of equal strength, dearest~",
      "such perfect balance, just like old times~",
      "shall we call it a draw, my sweet?~",
      "how nostalgic, matching powers like this~"
    ],
    // Soul, Heart & Sky Tie Messages
    'sunlight': [
      "How peculiar... something matches my power?",
      "A worthy addition to my collection.",
      "Finally, a being that understands true power.",
      "Now THIS is interesting..."
    ],
    ringmaster: [
      "NOW THIS IS A PROPER SHOWDOWN!",
      "TWO STARS FOR THE PRICE OF ONE!",
      "THE AUDIENCE DEMANDS A REMATCH!"
    ],
    sole: [
      "Our lights shine as one...",
      "Darkness breeds unity...",
      "Together we illuminate..."
    ],
    blitz: [
      "A worthy corruption...",
      "Our fall mirrors each other...",
      "Tainted grace recognizes its own..."
    ],
    hitbox: [
      "Calculations... inconclusive.",
      "A variable I hadn't considered.",
      "Processing new combat data..."
    ],
    goldie: [
      "A costly stalemate...",
      "This wasn't in the budget...",
      "Time to renegotiate!"
    ],
    leopold: [
      "Oh... um... good job!",
      "Your kindness matches mine!",
      "We can both be nice AND strong!"
    ],
    frost: [
      "Our powers balance...",
      "Neither melts nor freezes...",
      "A perfect winter's day..."
    ],
    lavender: [
      "The garden watches us both...",
      "Two seeds of equal strength...",
      "Growth requires balance..."
    ]
  },
  pairs: {
    'terra_penelope': ["Cat party! Meow~", "Best friends forever!"],
    'justice_penelope': ["Heroes in training!", "Justice and joy unite!"],
    'enraged_kite_kite': ["STOP COPYING ME!", "I HATE MYSELF!"],
    'high_kite_kite': ["No tricks needed.", "Time to grow up."],
    'null_awareness': ["U̷N̷D̷E̷F̷I̷N̷E̷D̷", "∞ ERROR ∞"],
    'justice_ginger': ["Violence isn't always the answer!", "Guns and capes don't mix."],
    'ginger_terra': ["...Put the gun down.", "Stop being cute, I'm trying to work."],
    'sally_ginger': ["Nice shot.", "Professional courtesy."],
    'kc_penelope': ["Best friends forever!", "Playtime buddies!"],
    'salvo_ginger': ["Just like old times!", "Partners in combat!"],
    'terra_kc': ["Nature's best friends!", "Guardians of joy!"],
    'justice_sally': ["A true defender of peace!", "Your skills are hero material!"],
    'prime_reflection': ["Immortal meets eternal...", "Two constants in chaos."],
    'worendy_null': ["C̷u̷r̷s̷e̷d̷ ̷g̷l̷i̷t̷c̷h̷", "V̷o̷i̷d̷ ̷i̷n̷k̷"],
    'maturity_carlgpt': ["lol ur code sux", "git gud scrub"],
    'zero_awareness': ["Nothing equals infinity.", "Void meets limitless.\nRussian limitless...?"],
    'echo_nova': ["Our resonance creates stars!", "Perfect harmony of space!"],
    'salvo_nova': ["Welp, your power's only as good as its creator.", "Yeah, yeah, Nova."],
    'kc_terra': ["Nature's best friends!", "Guardians of joy!"],
    'penelope_kc': ["Double the happiness!", "Friends forever and ever!"],
    'high_kite_enraged_kite': ["I hate what I've become.", "We're both broken inside."],
    'justice_prime': ["Eternal justice prevails!", "An immortal hero rises!"],
    'cupiditas_null': ["Shadows meet void.", "Darkness recognizes darkness, Sune."],
    'feathers_penelope': ["Mother and daughter stand together!", "Hihi, penelope!"],
    'feathers_highpoint': ["I fight myself...", "Two sides of the same protector."],
    'feathers_terra': ["Nature's guardians unite!", "We both protect in our own ways."],
    'highpoint_justice': ["Your justice matches my determination!", "You remind me of... ugh... the hatted one."],
    'subject_192_penelope': ["<3  GFbgd", "penny <3?"],
    'subject_192_kc': ["gfvnnn", "KC nice <3"],
    'subject_192_feathers': ["fffdsd", "mama?"],
    'fevaa_binx': ["Best friends forever!", "Wenbinx!"],
    'fevaa_penelope': ["Hey there, kid!", "Proving looks can be deceiving!"],
    'binx_justice': ["Your sense of justice needs work.", "Can't take a divine joke?"],
    'cyrus_subject_192': [
      "Hello, Dean!",
      "Good game!",
      "Wanna play Roblox after this?"
    ],
    'drew_awareness': ["HAHA! I stand against infinity!", "THIS IS EPIC!"],
    'drew_null': ["What do you mean 'error'? Fight me properly!", "Stop glitching and face me!"],
    'drew_maturity': ["At least act your power level...", "This is beneath me."],
    'drew_highpoint': ["Finally, a REAL challenge!", "Now THIS is more my speed!"],
    'knight_moonlight': [
      "My lady... is that really you?",
      "Please... remember me...",
      "Your corruption pains me so."
    ],
    'knight_null': [
      "You remind me of her... the void...",
      "Another corrupted being...",
      "We are both trapped here."
    ],
    'knight_awareness': [
      "Your infinite power cannot free me.",
      "Even omniscience has its limits.",
      "Knowledge of all, yet you can't restore her..."
    ],
    'blossom_herb': ["The garden's harmony...", "Life and death entwined..."],
    'ciph_soiley': ["The deep ones awaken...", "Darkness feeds darkness..."],
    'evergreen_terra': ["Two gardens meet...", "Nature's duality..."],
    'blossom_evergreen': ["The garden grows stronger...", "Seeds of power bloom..."],
    'herb_ciph': ["Bone and thorn unite...", "Death's garden flourishes..."],
    'allie_worendy': [
      "PRODUCT OF MY DESIGN.",
      "PRODUCT OF A DESIGN.",
      "DRAWN OF ECHOING SKY.",
      "PRODUCT OF HIS DESIGN..."
    ],
    'ember_aqua': [
      "Fire and water in perfect balance...",
      "Even opposites can find harmony.",
      "Elements clash and unite!"
    ],
    'ember_salvo': [
      "Your flames match mine!",
      "Together we set the world ablaze!",
      "Fire elementals unite!"
    ],
    'ember_fester': [
      "Divine fire meets elemental flame!",
      "Two flames, one inferno!",
      "The fire grows stronger!"
    ],
    'aqua_salvo': [
      "Your flames resist my waves...",
      "Water and fire dance eternally.",
      "Neither element yields!"
    ],
    'yelpo_penelope': [
      "Best friends forever! ...right?",
      "Hey, Penny!",
      "Together, even if we don't know why!"
    ],
    'blueberry_kite': [
      "Partners in crime!",
      "Double trouble!",
      "The perfect trick duo!"
    ],
    'subject_192_vicki': [
      "dghcfhdbdb :D",
      "gb gvnfgngn!"
    ],
    'fossil_subject_192': [
      "Sanctuary found in darkness.",
      "Light and shadow in harmony."
    ],
    'vicki_fossil': [
      "Hi!",
      "Angels and aliens unite!"
    ],
    'sunlight_zero': [
      "Fascinating... the void grows strong enough to match even me.",
      "The emptiness becomes fullness... how curious.",
      "Zero and infinity... two sides of the same coin."
    ],
    'hitbox_binx': ["Stop moving so fast!", "Hold still, you annoying angel!"],
    'hitbox_maturity': ["Your power... HOW?", "These numbers don't add up!"],
    'hitbox_ember': ["Systems overheating...", "Fire detected in targeting system!"],
    'frost_fester': ["Fire and ice dance eternally...", "Neither element yields..."],
    'lavender_blossom': ["The garden's secrets deepen...", "Two faces of nature's truth..."],
    'leopold_feathers': ["We protect in different ways!", "Kindness has many forms..."],
    'goldie_drew': ["My wealth exceeds your ego!", "Money beats attitude!"],
    'ringmaster_justice': ["A HERO IN MY SHOW!", "WHAT SPECTACULAR JUSTICE!"],
    'sole_subject_192': ["Light meets shadow...", "Darkness bears unexpected friends..."],
    'blitz_knight': ["Another fallen warrior...", "We both lost something divine..."]
  }
};